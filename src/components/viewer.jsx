...
import { setRealViewPort, sendMouseWheel, setFill } from "../actions";
...
class Viewer extends React.Component {
	...
	notifyRealImagePos() {
		let zoom = this.props.api.getRealScale(this.scale, this.level);
		let dims = this.props.api.getRealImagePos(this.imagePos, this.scale, this.level);
	...
	receiveNewState() {
		if(this.state.realViewPort.reposition) {
			let {w, h} = this.props.api.getRealImagePos(this.imagePos, this.scale, this.level);
	...
	determineZoomFactor(delta) {
		let rev = delta > 0 ? -1 : 1;
		let rs = this.props.api.getRealScale(this.scale, this.level);
